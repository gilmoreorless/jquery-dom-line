 ...
 */
(function(a,b){function e(b,c,e){var f=[b.x,b.y,c.x,c.y,e.w,e.h].join(",");if(d[f])return d[f];var g=Math.abs(c.x-b.x),h=Math.abs(c.y-b.y),i=!g||!h?g||h:Math.sqrt(g*g+h*h),j=Math.min(b.x,c.x),k=Math.min(b.y,c.y),l=j+g/2,m=k+h/2,n,o=l-i/2,p=m,q=d[f]={width:i};e.w>1&&(q.width-=e.w-1),e.h>1&&(p-=e.h/2),o=Math.round(o-e.l),p=Math.round(p-e.t),q.width=Math.round(q.width),g?h?n=(180+Math.atan2(b.y-c.y,b.x-c.x)*180/Math.PI+360)%360:n=b.x<c.x?0:180:n=b.y<c.y?90:270,q.transform="translate("+o+"px,"+p+"px) rotate("+n+"deg)",q.extra={center:{x:l,y:m},rotation:{deg:n,rad:n%360*Math.PI/180}};return a.extend({},q)}function c(a){if(a.x===b&&a.y===b)return!1;a.x=parseFloat(a.x)||0,a.y=parseFloat(a.y)||0;return a}var d={};a.line=function(b,d,f){b=c(b),d=c(d);if(!b||!d)return!1;var g=a.extend({},a.line.defaults,f||{}),h=g.elem?a(g.elem):a("<div/>",{"class":g.className}),i={position:"absolute",backgroundColor:g.lineColor,width:1,height:g.lineWidth},j,k,l=h;h.css(i),h[0].parentNode||h.appendTo("body"),j=e(b,d,{w:h.outerWidth(),h:h.outerHeight(),l:parseFloat(h.css("marginLeft"))||0,t:parseFloat(h.css("marginTop"))||0}),k=j.extra,delete j.extra,h.css(j),g.returnValues&&(l={from:b,to:d,center:k.center,rotation:k.rotation});return l},a.line.defaults={elem:"",className:"jquery-line",lineWidth:1,lineColor:"#000",returnValues:!1}})(jQuery)
/*
